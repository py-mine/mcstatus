...
class TestBuffer:
    ...
    def test_clear_only_already_read(self) -> None:
        self.connection.write(b"abcdef")
    ...
    def test_unread_view(self) -> None:
        self.connection.write(b"abcdef")
        assert self.connection.read(2) == b"ab"

        assert bytes(self.connection.unread_view()) == b"cdef"

    def test_flush_only_returns_unread_data(self) -> None:
        self.connection.write(b"abcdef")
        assert self.connection.read(2) == b"ab"
    ...
    def test_varint_cases(self) -> None:
    ...
    def test_read_invalid_varint(self) -> None:
    ...
    def test_write_invalid_varint(self) -> None:
        with pytest.raises(
            ValueError,
    ...
    def test_write_long_negative(self) -> None:
    ...
    def test_write_long_positive(self) -> None:
    ...
    def test_read_ulong_positive(self) -> None:
        self.connection.write(bytearray.fromhex("8000000000000000"))

        assert self.connection.read_value(StructFormat.ULONGLONG) == 9223372036854775808

    def test_write_ulong_positive(self) -> None:
    ...
    def test_read_bytearray(self) -> None:
    ...
    def test_read_not_enough(self) -> None:
    ...
    def test_read_negative_length(self) -> None:
...
class TestTCPSocketConnection:
    ...
    def test_read_not_enough(self, connection: TCPSocketConnection) -> None:
        cast("Mock", connection.socket.recv).side_effect = [bytearray(b"a"), bytearray()]

        with pytest.raises(IOError, match=r"^Server did not respond with any information!$"):
            _ = connection.read(2)

    def test_write(self, connection: TCPSocketConnection) -> None:
...
class TestUDPSocketConnection:
    ...
    def test_remaining(self, connection: UDPSocketConnection) -> None:
        assert connection.remaining == 65535

    def test_read(self, connection: UDPSocketConnection) -> None:
        cast("Mock", connection.socket.recvfrom).return_value = [bytearray.fromhex("7FAA")]

        assert connection.read(2) == bytearray.fromhex("7FAA")

    def test_read_empty(self, connection: UDPSocketConnection) -> None:
        cast("Mock", connection.socket.recvfrom).return_value = []

        with pytest.raises(IndexError, match=r"^list index out of range$"):
            _ = connection.read(1)

    def test_write(self, connection: UDPSocketConnection) -> None:
        connection.write(bytearray.fromhex("7FAA"))
